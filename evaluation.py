...
bot_script_csv = {
	"location":	"Hi, I am Glue, a social bot. I am looking forward to talking to you today. Where are you both right now?",
...
def get_conv_details(log):
	...
	for key in turn_counter:
		if key != "GLUE":
			...
			if count == 0:
				score_1 = turn_counter[key]/turn_counter["GLUE"]
				count = count + 1
			if count == 1:
				score_2 = turn_counter[key]/turn_counter["GLUE"]
	composite_score= composite_score/turn_counter["GLUE"]
	print(composite_score)
...
'''
def coherence_metric(UI_log, script):
	...
	for i in range(UI_log.shape[0]):
		...
		if "glue keep quiet" in str(UI_log[5][i]):
			if UI_log[4][i] != "silent_response":
				coherence_metric_3 = coherence_metric_3 + 1
				print("silent response not used when needed in log entry {0} {1}".format(i+4, UI_log[0][i]))
		if "glue respond" in str(UI_log[5][i]):
			if UI_log[4][i] == "silent_response":
				coherence_metric_3 = coherence_metric_3 + 1
	...
	for i in range(len(intents_detected_list)):
		if intents_list[script_index] in intents_detected_list[i]:
			...
			if intents_detected_list[i] in intents_list:
				print("error intent not in order")
	...
	return coherence_metric_1, coherence_metric_2, coherence_metric_3
...
def evaluation(log, script):
	glue_turns, score_1, score_2, turn_counter, glue_icebreaker, alana_bot = get_conv_details(log)
	duration, time_per_glue_turn = get_total_conv_time(log)
	coherence_metric_1, coherence_metric_2, coherence_metric_3 = coherence_metric(log, script)
	return turn_counter, score_1, score_2, glue_icebreaker, alana_bot, duration, time_per_glue_turn, coherence_metric_1, coherence_metric_2, coherence_metric_3
...
def run():
	...
	return rg_turn_counter, rg_score_1, rg_score_2, rg_glue_icebreaker, rg_alana_bot, rg_duration, rg_coherence_metric_1, rg_coherence_metric_2, rg_coherence_metric_3, cc_turn_counter, cc_score_1, cc_score_2, cc_glue_icebreaker, cc_alana_bot, cc_duration, cc_coherence_metric_1, cc_coherence_metric_2, cc_coherence_metric_3, rg_avg_rating, rg_response_rating, cc_avg_rating, cc_response_rating, likert_rating
